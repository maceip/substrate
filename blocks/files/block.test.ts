// block.test.ts — invariants of the files block. Dependency-free; run with node.
//   node files/block.test.ts
//
...
await check('port invariance: memory, fs and object agree on app behavior', async () => {
  ...
  assert.equal(memory, run('fs'), 'memory and fs diverged')
  assert.equal(memory, run('object'), 'memory and object diverged')
})
...
await check('gate escalation: nursery -> elementary -> graduated', () => {
  ...
  assert.equal(evaluate({ ...base, signals: { prodData: false, publicDownloads: false, instances: 1, objectCount: 10, totalBytes: 1_000 } }).requiredGrade, 'nursery')
  assert.equal(evaluate({ ...base, signals: { prodData: true, publicDownloads: false, instances: 1, objectCount: 10, totalBytes: 1_000 } }).requiredGrade, 'elementary')
  assert.equal(evaluate({ ...base, signals: { prodData: true, publicDownloads: true, instances: 4, objectCount: 90_000, totalBytes: 6_000_000_000 } }).requiredGrade, 'graduated')
})
